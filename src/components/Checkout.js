              ...
              {/* TODO: CRIO_TASK_MODULE_CHECKOUT - Display list of addresses and corresponding "Delete" buttons, if present, of which 1 can be selected */}
               {addresses.all.length===0?<Typography my="1rem">
                ...
                addresses.all.map((addr)=>{
                  ...
                  return (
                    <Box className={addresses.selected===addr._id?'address-item selected':"address-item not-selected"} key={addr._id} onClick={()=>{setAddresses({...addresses,selected: addr._id})}}>
                      <p>{addr.address}</p>
                      <Button startIcon={<Delete />} onClick={async () => await deleteAddress(token,addr._id) }>
                        Delete
                      </Button>
                    </Box>
                  )
